...
<div class="quiz-form-container">
    ...
    <h1>Update Quizzes</h1>

    <form action="{{ url('updateallquiz',$data->id) }}" method="POST">
        @csrf

        <label for="name">Quiz Title</label>
        <input type="text" name="name" id="name" value="{{ $data->name }}" placeholder="Enter quiz name" required>

        <label for="category">Category</label>
        <select name="category_id" id="category" required>
            <option value="">Select Category</option>
            @foreach ($cateogry as $items)
                <option value="{{ $items->category_id  }}">{{ $items->name }}</option>
            @endforeach
        </select>
